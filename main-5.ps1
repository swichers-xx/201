...
function Ping-Host {
  ...
  $wmi=Get-CimInstance -Query $query
  Write-Output $wmi
}
...
function Get-OS {
  ...
  $wmi=Get-CimInstance -ClassName Win32_OperatingSystem -ComputerName $computername -ErrorAction Stop
...
$form1.Controls.Add($servicesDataGridView) 
...
$dataGridView.Add_SelectionChanged($UpdateServicesGrid) 

# In GenerateForm function

$servicesDataGridView.Name = "servicesDataGridView"
$servicesDataGridView.Dock = "Bottom"
$servicesDataGridView.ReadOnly = $true

# In UpdateServicesGrid script block
$selectedServer = $dataGridView.SelectedRows[0].Cells[0].Value
# get services
$servicesDataGridView.DataSource = $services

# After GenerateForm function
$servicesDataGridView.AutoResizeColumns() 
$servicesDataGridView.Columns['Status'].Width = 60
$servicesDataGridView.Columns['Name'].Width = 200
