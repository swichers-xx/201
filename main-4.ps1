...
function Get-VoxcoServicesStatus {
  Param([string]$computername=$(Throw "You must specify a computername."))
  Write-Debug "In Get-VoxcoServicesStatus Function"
  $services = Get-Service -ComputerName $computername | Where-Object { $_.Name -like "*Voxco*" }
  $servicesStatus = $services | ForEach-Object { "$($_.Name): $($_.Status)" }
  return $servicesStatus -join ", "
}

function Restart-ComputerRemote {
  Param([string]$computername=$(Throw "You must specify a computername."))
  Write-Debug "In Restart-ComputerRemote Function"
  Restart-Computer -ComputerName $computername -Force
}

function Stop-VoxcoServices {
  Write-Debug "In Stop-VoxcoServices Function"
  Get-Service | Where-Object { $_.Name -like "*Voxco*" } | Stop-Service -Force
}

function Start-VoxcoServices {
  Write-Debug "In Start-VoxcoServices Function"
  Get-Service | Where-Object { $_.Name -like "*Voxco*" } | Start-Service
}

function Restart-VoxcoServices {
  Write-Debug "In Restart-VoxcoServices Function"
  Get-Service | Where-Object { $_.Name -like "*Voxco*" } | Restart-Service
}

function Get-DiskUsage {
  Param([string]$computername=$(Throw "You must specify a computername."))
  Write-Debug "In Get-DiskUsage Function"
  $disks=Get-WmiObject Win32_LogicalDisk -computername $computername -ea stop
  
  foreach ($disk in $disks) {
    $diskSize = $disk.Size / 1GB -as [int]
    $freeSpace = $disk.FreeSpace / 1GB -as [int]
    Write-Output "Disk $($disk.DeviceID): Size = $diskSize GB, Free Space = $freeSpace GB"
  }
}
...
if ($pingResult.StatusCode -eq 0) {
   ...
   # Get disk usage
   $diskUsage = Get-DiskUsage $computer
   $obj | Add-Member Noteproperty DiskUsage $diskUsage

   # Get Voxco services status
   $voxcoServicesStatus = Get-VoxcoServicesStatus $computer
   $obj | Add-Member Noteproperty VoxcoServicesStatus $voxcoServicesStatus
}
          }
          else {
...
$btnRestart.add_Click({ Restart-ComputerRemote -computername $txtComputerList.Text })
